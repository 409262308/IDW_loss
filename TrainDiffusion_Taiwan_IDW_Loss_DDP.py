...
def training_step(model, loss_fn, optimiser, data_loader, scaler, epoch, accum, writer, device):
    model.train()
...
@torch.no_grad()
def validation_step(model, loss_fn, data_loader, epoch, device):
    model.eval()
    losses = []
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = training_step(network, loss_fn, optimiser, dataloader_train, scaler, epoch, args.accum, writer, device)
        val_loss = validation_step(network, loss_fn, dataloader_val, epoch, device)
