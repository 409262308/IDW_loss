...
def sample_model_eds_batch(
    ...
    coarse = batch["coarse"]
    condition_params = torch.stack((batch["year"].to(device), batch["doy"].to(device)), dim=1)
...
def main() -> None:
    ...
    if merged_config.get("label_features") != ["year", "doy"]:
        raise ValueError(
            "This inference code requires a checkpoint trained with "
            "label_features=['year', 'doy']. Retrain with the updated training script."
        )
    if merged_config.get("network_conditioning") != "unscaled_condition_v1":
        raise ValueError(
            "This inference code requires a checkpoint trained with the updated "
            "conditional EDM preconditioning. Retrain before inference."
        )
    ...
    expected_spatial_mask = "full_grid" if no_mask else "land"
    if stats.get("spatial_mask") != expected_spatial_mask:
        raise ValueError(
            f"Stats spatial_mask={stats.get('spatial_mask')!r}, expected "
            f"{expected_spatial_mask!r}. Use stats generated by the matching training run."
        )
    if stats.get("resolution") != args.resolution:
        raise ValueError(
            f"Stats resolution={stats.get('resolution')!r}, expected {args.resolution!r}."
        )
    if stats.get("target_transform") != target_transform:
        raise ValueError(
            f"Stats target_transform={stats.get('target_transform')!r}, expected "
            f"{target_transform!r}."
        )
    if int(stats.get("tp_idw_k", -1)) != tp_idw_k or not math.isclose(
        float(stats.get("tp_idw_power", float("nan"))),
        tp_idw_power,
        rel_tol=0.0,
        abs_tol=1e-12,
    ):
        raise ValueError("Stats IDW settings do not match the inference IDW settings.")
    ...
    print0(
        ...
        f"valid_pixels={int(dataset_test.valid_mask.sum().item())}"
    )
